...
int main()
{
  ...
  while(cin>>n)
  {
    ...
    for(int i=0;i<n;i++)
    {
      for(int j=i+1;j<n;j++)
      {
        if(SegmentIntersection(Q[i].first,Q[i].second,Q[j].first,Q[j].second,a,b))
        {
          for(int z=j+1;z<n;z++)
          {
            if(SegmentIntersection(Q[i].first,Q[i].second,Q[z].first,Q[z].second,a,b) &&
            SegmentIntersection(Q[j].first,Q[j].second,Q[z].first,Q[z].second,a,b))
            {
              sum++;
            }
          }
