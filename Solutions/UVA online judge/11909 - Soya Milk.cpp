...
// a = max x, b = max y from the center
...
point rotate(point p, double rad, point ctr) {
    p=p-ctr;
    point aux(p.x * cos(rad) - p.y*sin(rad),
...
int32_t main() {
    ...
    while(cin>>l>>w>>h>>angle){
        ...
        point p(l,h),ctr(0,0),a(0,h),b,aux2(l,0);
        p=rotate(p,angle,ctr);
        a=rotate(a,angle,ctr);
        aux2=rotate(aux2,angle,ctr);

        line l1,l2,l3;
        point aux(a.x+1,a.y);
        pointsToLine(a,aux,l1);
        pointsToLine(p,aux2,l2);
        pointsToLine(ctr,aux2,l3);
        areIntersect(l1,l2,b);
        if(b.y<aux2.y)
        {
            areIntersect(l1,l3,b);
            cout<<fixed<<setprecision(3)<<area(ctr,a,b)*w<<" mL\n";
        }
        else{
            db total=w*l*h;


            cout<<fixed<<setprecision(3)<<total-area(p,a,b)*w<<" mL\n";
        }
    }
    return 0;
}
