...
//https://onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&page=show_problem&problem=1518
...
int32_t main() {
    ...
    while(cin>>n)
    {
        ...
        circumCircle(a1,a2,a3,ctr,r);
        db angle=(2*PI/n),minx=1e10,maxx=-1e10,miny=1e10,maxy=-1e10;
        point p=a1;
        for(int i=0;i<n;i++)
        {
            maxx=max(maxx,p.x);
        ...
        cout<<fixed<<setprecision(3)<<"Polygon "<<k<<": "<<(maxx-minx)*(maxy-miny)<<"\n";
        k++;
